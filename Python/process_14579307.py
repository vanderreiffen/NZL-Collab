...
def proceed_14579307(dictfiletorun, obsfullpath, wbmain, source_path, master_path):
    ...
    if not (dictfiletorun["DownloadedFilePaths"] == 'None'):
        ...
        try:
            scdownload, status = update_14579307_10001(str(dictfiletorun["DownloadedFilePaths"]), obsfullpath,
        ...
        try:
            scdownload = ""
            scdownload, status = goWebsite_14579307_10001(DictFrmObs, dictfiletorun, source_path, master_path,
                                                              pub_updt, methodID)
        except Exception as e:
            status = str(e)
        ...
        if scdownload != "" and status == "":
            ...
            try:
                scdownload, status = update_14579307_10001(str(scdownload), obsfullpath,
                                                 str(dictfiletorun["Freq."]), str(dictfiletorun["QCpath"]),
                                                DictFrmObs[pub_updt][3], dictMD, dictRoundg)
            except:
                status = "Failed - <Source File Layout/Content Change>"
...
def goWebsite_14579307_10001(DictFrmObs, dictfiletorun, scpath, master_path, publication, smid):
    finscname = "";
    strerror = ""
    res = False
    # ----------------------------------------------------------------------------------------------
    ...
    try:
        try:
            # 1st page
            for el in  driver.find_elements_by_tag_name('div'):
                if xt(str(el.get_attribute('innerText'))).strip().find(xt("Expand All")) >= 0:
                    el.click()
                    time.sleep(3)
            ...
            for el in  driver.find_elements_by_tag_name('a'):
                if xt(str(el.get_attribute('innerText'))).strip().find(xt("Production of sawn timber, 1970 to most recent")) >= 0:
...
def update_14579307_10001(scdownloaded, obsfilepath, freq, path_to_save, dictUpdateIDsfrmSc, dictMD, dictRoundg):
    # global link_folder
    global dictSCDate
    dictcol = {}
    obs = xlz.gencache.EnsureDispatch("Excel.Application")
...
def get_data_date_101(sh, tofind, scbk):
    ...
    try:
        ...
        rng_col = find_mapping_2(sh, xt(tofind),'|')
        if rng_col is not None:
            icol = rng_col.Column
            print(icol)
            # row / date
            rng_row =sh.Cells(1,1)
            while rng_row.Row <= 500:
                initdata = rng_row.Text
                if initdata != "":
                    if is_year(left(initdata,4)):
                        iyr = left(initdata,4)
                        im = 3
                        if is_date(iyr,im):
                            irow = rng_row.Row
