...
def proceed_14609117(dictfiletorun, obsfullpath, wbmain, source_path, master_path):
    ...
    if not (dictfiletorun["DownloadedFilePaths"] == 'None'):
        ...
        try:
            scdownload, status = update_14609117_10001(str(dictfiletorun["DownloadedFilePaths"]), obsfullpath,
        ...
        try:
            scdownload = ""
            scdownload, status = goWebsite_14609117_10001(DictFrmObs, dictfiletorun, source_path, master_path,
                                                              pub_updt, methodID)
        except Exception as e:
            status = str(e)
        ...
        if scdownload != "" and status == "":
            ...
            try:
                scdownload, status = update_14609117_10001(str(scdownload), obsfullpath,
                                                 str(dictfiletorun["Freq."]), str(dictfiletorun["QCpath"]),
                                                DictFrmObs[pub_updt][3], dictMD, dictRoundg)
            except:
                status = "Failed - <Source File Layout/Content Change>"
...
def goWebsite_14609117_10001(DictFrmObs, dictfiletorun, scpath, master_path, publication, smid):
    finscname = "";
    strerror = ""
    res = False
    # ----------------------------------------------------------------------------------------------
    ...
    try:
        try:
            ...
            ws = scBk.Worksheets[1]
            ws.Name = "Data"
            rng  = None
            ws.Cells(2,1).Value = "P/E"
            dataVal = ''

            now = datetime.datetime.now()
            datadate = datetime.datetime(now.year, now.month, now.day+1)
            ws.Cells(1,1).NumberFormat = "dd-mmm-yyyy"
            ws.Cells(1, 1).Value = datadate
            res = False
            for el in driver.find_elements_by_tag_name('td'):
                if xt(str(el.get_attribute('innerText'))).strip().find(xt("P/E")) >= 0:
                    parent_el = el.find_element_by_xpath("../../..")
                    break

            for el in parent_el.find_elements_by_tag_name('td'):
                if xt(str(el.get_attribute('class'))).strip() == "text-right":
                    dataVal =  xt(str(el.get_attribute('innerText'))).strip()
                    ws.Cells(3, 1).Value = dataVal
                    res = True
                    break

            if res:
                scBk.SaveAs(DestinationFile.replace("/", '\\'))
            else:
...
def update_14609117_10001(scdownloaded, obsfilepath, freq, path_to_save, dictUpdateIDsfrmSc, dictMD, dictRoundg):
    # global link_folder
    global dictSCDate
    dictcol ={}
    obs = xlz.gencache.EnsureDispatch("Excel.Application")
