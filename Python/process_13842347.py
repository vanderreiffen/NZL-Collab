...
def proceed_13842347(dictfiletorun, obsfullpath, wbmain, source_path, master_path):
    ...
    if not (dictfiletorun["DownloadedFilePaths"] == 'None'):
        ...
        try:
            scdownload, status = update_13842347_10001(str(dictfiletorun["DownloadedFilePaths"]), obsfullpath,
        ...
        try:
            scdownload = ""
            scdownload, status = goWebsite_13842347_10001(DictFrmObs, dictfiletorun, source_path, master_path,
                                                              pub_updt, methodID)
        except Exception as e:
            status = str(e)
        ...
        if scdownload != "" and status == "":
            ...
            try:
                scdownload, status = update_13842347_10001(str(scdownload), obsfullpath,
                                                 str(dictfiletorun["Freq."]), str(dictfiletorun["QCpath"]),
                                                DictFrmObs[pub_updt][3], dictMD, dictRoundg)
            except Exception as e:
                status = str(e)
...
def goWebsite_13842347_10001(DictFrmObs, dictfiletorun, scpath, master_path, publication, smid):
    finscname = "";
    strerror = ""
    res = False
    # ----------------------------------------------------------------------------------------------
    ...
    try:
        try:
            ...
            for el in  driver.find_elements_by_tag_name('a'):
                if xt(str(el.get_attribute('innerText'))).strip().find(xt("International visitor survey key data year ending")) >= 0:
                    el.click()
                    time.sleep(3)
...
def update_13842347_10001(scdownloaded, obsfilepath, freq, path_to_save, dictUpdateIDsfrmSc, dictMD, dictRoundg):
    # global link_folder
    global dictSCDate
    dictcol = {}
    obs = xlz.gencache.EnsureDispatch("Excel.Application")
...
def get_data_date_101(sh, tofind):
    ...
    try:
        # cycle sheet
        set_sheet(sh)
        rng_row = find_mapping(sh, xt(tofind),':')
        if rng_row is not None:
            irow = rng_row.Row
            # col / date
            rng_col = find_mapping(sh, "year ended",":")
            if rng_col is not None:
                while rng_col.Column <= 10:
                    gtxt = xt(rng_col.Text)
                    if gtxt != '':
                        if gtxt.find('year ended') >= 0:
                            iyr = only_digits(gtxt)
                            im = 9
                            if is_date(iyr,im):
                                icol = rng_col.Column
