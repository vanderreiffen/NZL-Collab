...
def proceed_14560387(dictfiletorun, obsfullpath, wbmain, source_path, master_path):
    ...
    if not (dictfiletorun["DownloadedFilePaths"] == 'None'):
        ...
        try:
            scdownload, status = update_14560387_10001(str(dictfiletorun["DownloadedFilePaths"]), obsfullpath,
        ...
        try:
            scdownload = ""
            scdownload, status = goWebsite_14560387_10001(DictFrmObs, dictfiletorun, source_path, master_path,
                                                              pub_updt, methodID)
        except Exception as e:
            status = str(e)
        ...
        if scdownload != "" and status == "":
            ...
            try:
                scdownload, status = update_14560387_10001(str(scdownload), obsfullpath,
                                                 str(dictfiletorun["Freq."]), str(dictfiletorun["QCpath"]),
                                                DictFrmObs[pub_updt][3], dictMD, dictRoundg)
            except:
                status = "Failed - <Source File Layout/Content Change>"
...
def goWebsite_14560387_10001(DictFrmObs, dictfiletorun, scpath, master_path, publication, smid):
    finscname = "";
    strerror = ""
    res = False
    # ----------------------------------------------------------------------------------------------
        ...
        try:
            ...
            for el in  driver.find_elements_by_tag_name('a'):
                if str(el.get_attribute('href')).strip().lower().find(xt(pubname))>=0 and \
                        str(el.get_attribute('href')).strip().lower().find(xt(".pdf")) >= 0:
                    el.click()
                    time.sleep(3)
...
def update_14560387_10001(scdownloaded, obsfilepath, freq, path_to_save, dictUpdateIDsfrmSc, dictMD, dictRoundg):
    # global link_folder
    global dictSCDate
    dictcol = {}
    obs = xlz.gencache.EnsureDispatch("Excel.Application")
    ...
    try:
        ...
        if rng_tbl is not None:
            ...
            if rng_row is not None:
                irow = rng_row.Row
                # dates / column
                rng_col = find_mapping(sh,xt('calendar years'),':',rng_tbl.Row)
                if rng_col is not None:
                    while rng_col.Column <= 30:
                        icol =0
                        if is_year(only_digits(rng_col.Text)):
                            iyr = only_digits(rng_col.Text)
                            im = 12
                            if is_date(iyr,im):
                                icol = rng_col.Column
    ...
    try:
        ...
        if rng_tbl is not None:
            ...
            if rng_row is not None:
                irow = rng_row.Row
                # dates / column
                rng_col = find_mapping(sh,xt('fx rates'),':',rng_tbl.Row)
                if rng_col is not None:
                    while rng_col.Column <= 30:
                        icol = 0
                        if xt(rng_col.Text).find('-')>=0:
                            iyr = only_digits(right(xt(rng_col.Text),2))
                            iyr = int(iyr) + 2000
                            im = getmonth(xt(rng_col.Text))
                            if is_date(iyr,im):
                                mydate =datetime.datetime(int(iyr),int(im),1)
                                if mydate not in dictSCDate:
                                    icol = rng_col.Column
