...
def proceed_14261147(dictfiletorun, obsfullpath, wbmain, source_path, master_path):
    ...
    if not (dictfiletorun["DownloadedFilePaths"] == 'None'):
        ...
        try:
            scdownload, status = update_14261147_10001(str(dictfiletorun["DownloadedFilePaths"]), obsfullpath,
        ...
        try:
            scdownload = ""
            scdownload, status = goWebsite_14261147_10001(DictFrmObs, dictfiletorun, source_path, master_path,
                                                              pub_updt, methodID)
        except Exception as e:
            status = str(e)
        ...
        if scdownload != "" and status == "":
            ...
            try:
                scdownload, status = update_14261147_10001(str(scdownload), obsfullpath,
                                                 str(dictfiletorun["Freq."]), str(dictfiletorun["QCpath"]),
                                                DictFrmObs[pub_updt][3], dictMD, dictRoundg)
            except:
                status = "Failed - <Source File Layout/Content Change>"
...
def goWebsite_14261147_10001(DictFrmObs, dictfiletorun, scpath, master_path, publication, smid):
    finscname = "";
    strerror = ""
    res = False
    # ----------------------------------------------------------------------------------------------
...
def update_14261147_10001(scdownloaded, obsfilepath, freq, path_to_save, dictUpdateIDsfrmSc, dictMD, dictRoundg):
    # global link_folder
    global dictSCDate
    dictcol = {}
    obs = xlz.gencache.EnsureDispatch("Excel.Application")
