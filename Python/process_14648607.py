...
def proceed_14648607(dictfiletorun, obsfullpath, wbmain, source_path, master_path):
    ...
    if not (dictfiletorun["DownloadedFilePaths"] == 'None'):
        ...
        try:
            scdownload, status = update_14648607_10001(str(dictfiletorun["DownloadedFilePaths"]), obsfullpath,
        ...
        try:
            scdownload = ""
            scdownload, status = goWebsite_14648607_10001(DictFrmObs, dictfiletorun, source_path, master_path,
                                                              pub_updt, methodID)
        except Exception as e:
            status = str(e)
        ...
        if scdownload != "" and status == "":
            ...
            try:
                scdownload, status = update_14648607_10001(str(scdownload), obsfullpath,
                                                 str(dictfiletorun["Freq."]), str(dictfiletorun["QCpath"]),
                                                DictFrmObs[pub_updt][3], dictMD, dictRoundg)
            except Exception as e:
                status = str(e)
...
def goWebsite_14648607_10001(DictFrmObs, dictfiletorun, scpath, master_path, publication, smid):
    finscname = "";
    strerror = ""
    res = False
    # ----------------------------------------------------------------------------------------------
    ...
    try:
        try:
            ...
            for el in  driver.find_elements_by_tag_name('a'):
                if xt(str(el.get_attribute('innerText'))).strip().find(xt("View the PSI Time Series Data")) >= 0:
                    el.click()
                    time.sleep(3)
...
def update_14648607_10001(scdownloaded, obsfilepath, freq, path_to_save, dictUpdateIDsfrmSc, dictMD, dictRoundg):
    # global link_folder
    global dictSCDate
    dictcol = {}
    obs = xlz.gencache.EnsureDispatch("Excel.Application")
...
def get_data_date_101(sh, tofind, scbk):
    ...
    try:
        # cycle sheet
        sh = scbk.Worksheets(str(ch[0]).strip())
        set_sheet(sh)
        ...
        if rng_col is not None:
            ...
            while rng_row.Row <= 500:
                ...
                if initdata != "":
                    if getmonth(initdata)>0:
                        im = getmonth(initdata)
                    if im > 0:
                        iyr = int(right(initdata, 2))
                        iyr = 2000 + iyr
                    if is_date(iyr,im):
                        irow = rng_row.Row
